        ...
        if ($result->num_rows === 1) {
            ...
            if (password_verify($current_pass, $user['password']) || $current_pass === $user['password']) {
                ...
                if ($update_stmt->execute()) {
                    ...
                    $users_update_stmt = $conn->prepare("UPDATE users SET password = ? WHERE email = ?");
                    $users_update_stmt->bind_param("ss", $hashed_new_pass, $email);
                    $users_update_stmt->execute();
                    $users_update_stmt->close();
